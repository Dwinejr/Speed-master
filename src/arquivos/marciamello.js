...
$('.royalSlidesContainer li').filter(function () { return $(this).html() === '' }).remove();
...
$(document).ready(function () {
	...
	$( ".prateleira.vitrine ul li .productImage" ).each(function( index ) {
		if($(this).find("span").find('img').length > 1){
			$(this).hover(
			  function() {
	...
	if(check == true)
	{
	   // console.log('mobile')
		if($('body.home').length > 0){
			...
			$('.swiper-container,.swiper-slide').css({height: height_img});
			
			var height_blogHome = $('.blogHome img:visible').height();
			$('.blogHome').css({height: height_blogHome});

			var height_look = $('.lookBookHome img:visible').height();
			$('.lookBookHome').css({height: height_look});
		}; 

		if($('body.departamento').length > 0){
			var height_banner2 = $('.banner2 img:visible').height();
			// console.log(height_img);
			$('.banner2 #banner2, .banner2 #banner2-rotator, .banner2 #content-slider, .banner2 #banner2-rotator, .banner2 #image-gallery, .banner2 #content-slider, .banner2 .royalWrapper, .banner2 .royalSlider, .banner2 #slider-ajax-container').css({height: height_banner2});			
		}; 
